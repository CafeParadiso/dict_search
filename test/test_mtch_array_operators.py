...
def test_all():
    values = DictSearch()(data.complex_data, {"values": {"$all": {"$gt": 0.5}}})
    values = list(values)
    assert len(values) == 2


def test_all_eq():
    values = DictSearch()([{"values": [2, 1, 1]}, {"values": [1, 1, 1]}], {"values": {"$all": 1}})
    values = list(values)
    assert len(values) == 1


def test_any():
    values = DictSearch().__call__(data.complex_data, {"posts": {"$any": {"text": "tsm"}}})
    values = list(values)
    assert len(values) == 1
